...
async function migrateCSVToSupabase(userId) {
  ...
  const missingCategories = neededCategories.filter(name => !existingCategoryNames.includes(name));
  
  if (missingCategories.length > 0) {
    console.log(`🔧 Creating missing categories: ${missingCategories.join(', ')}`);
    ...
    const { data: updatedCategories } = await supabase
      ...
      .select('id, name')
      .eq('user_id', userId);
    categories.push(...updatedCategories.filter(c => missingCategories.includes(c.name)));
  }
  ...
  for (const filename of csvFiles) {
    try {
      ...
      for (const line of lines) {
        ...
        // Month header
        if (trimmed.includes(' - ') && (trimmed.includes('jt') || trimmed.includes('k'))) {
        ...
        // Skip headers
        if (trimmed.includes('HARI,BARANG') || trimmed.includes('DAILY') || trimmed.includes('MONTHLY')) continue;
      ...
      for (let i = 0; i < transactions.length; i += chunkSize) {
        ...
        if (error) {
          console.log(`❌ Error inserting chunk from ${filename}:`, error.message);
        }
      }
      
      console.log(`✅ ${filename}: ${transactions.length} transactions imported`);
