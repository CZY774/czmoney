...
const categoryKeywords = {
  'Food & Drink': ['nasi', 'mie', 'telur', 'beras', 'makanan', 'spaghetti', 'bakso'],
...
export async function POST({ request }) {
  ...
  try {
    ...
    for (const [year, lines] of Object.entries(sampleData)) {
      ...
      for (const line of lines) {
        ...
        if (monthMatch && monthMap[monthMatch[1]]) {
          ...
          if (income) {
            const incomeDate = `${year}-${monthMap[income.month].toString().padStart(2, '0')}-01`;
        ...
        if (parts.length >= 4) {
          ...
          if (day && description && amount && currentMonth) {
            ...
            if (date && amountNum > 0) {
              ...
              const fullDescription = `${description}${quantity ? ` (${quantity})` : ''}`;
    ...
    if (allIncomeEntries.length > 0) {
      ...
      if (incomeError) {
        return json({ error: `Income import failed: ${incomeError.message}` }, { status: 500 });
      }
    ...
    if (allTransactions.length > 0) {
      ...
      if (expenseError) {
        return json({ error: `Expense import failed: ${expenseError.message}` }, { status: 500 });
      }
    ...
    return json({
      ...
      total: totalImported,
      dateRange: `${allTransactions[0]?.txn_date} to ${allTransactions[allTransactions.length-1]?.txn_date}`
    });
