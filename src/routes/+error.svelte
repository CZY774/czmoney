  ...
  function getErrorInfo(status) {
    const errors = {
      ...
      503: {
        ...
        description: 'The service is temporarily down for maintenance. Please check back soon.',
        emoji: '🛠️',
        color: 'text-yellow-500'
      },
      504: {
    ...
    return errors[status] || {
      ...
      description: 'An unexpected error occurred. Please try refreshing the page or contact support.',
      emoji: '💥',
      color: 'text-gray-500'
    };
  }
  ...
  <div class="text-center space-y-8 p-8 max-w-2xl">
    ...
    <div class="space-y-4">
      ...
      {#if message}
        <details class="text-sm text-muted-foreground">
          <summary class="cursor-pointer hover:text-foreground inline-flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Technical Details
          </summary>
          <div class="mt-4 p-4 bg-card rounded-lg border border-border text-left">
    ...
    <div class="pt-8 border-t border-border">
      <p class="text-sm text-muted-foreground">
        {#if status === 401}
          Try <a href="/auth/login" class="text-primary hover:underline">signing in</a> or 
          <a href="/auth/register" class="text-primary hover:underline">creating an account</a>
        {:else if status === 404}
          Check the URL or browse from the <a href="/" class="text-primary hover:underline">home page</a>
        {:else if status >= 500}
